      ...
      <el-row>
        ...
        <el-form-item class="input" v-if="type!='info'"  label="分类" prop="fenlei">
          <el-input v-model="ruleForm.fenlei" 
              placeholder="分类" clearable ></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jingqufenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingqufenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
